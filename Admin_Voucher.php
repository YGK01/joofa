...
if (isset($_GET["page"])) {
    ...
    $get = "SELECT * FROM voucher ORDER BY VoucherID ASC LIMIT $start_from, $limit";
...
if(isset($_POST['upload'])){
    ...
    $date = $_POST['exdate'];
    $VAmount = $_POST['V_Amount'];
    $Vstatus = $_POST['V_Status'];

    $get1 = "SELECT * FROM voucher";
    
    $add = mysqli_query($con,$get1);
    $count = mysqli_num_rows($add);
    $newvoucher = sprintf("V%05d", $count + 1);
    
    if($Vcode=="" && $Vprice=="" && $date==""){
        echo '<script>alert("Please Fill up the form")</script>';
    }
    
    $get1 = "SELECT * FROM voucher WHERE VoucherCode = '$Vcode'" ;
    ...
    if($count>=1){
        echo '<script>alert("Voucher Code is Duplicated")</script>';
    }
    else{
        $insert = "INSERT INTO voucher VALUES ('". $newvoucher ."', '". $Vcode ."', '". $Vprice ."', '". $VAmount ."', 'Active', '". $date ."' )";
        mysqli_query($con,$insert);
    }
}

if(isset($_POST['randomcode'])){
...
<head>
    <title>Joo Fa Trading - Admin Voucher Page</title>
    <meta charset="utf-8">
                ...
                <section class="bg-light">
                    <div class="container pb-5">
                        <div class="left">
                            ...
                            <input type="text" name="V_Code" value="<?php echo $myRandomString ?>" />
                            <input type="submit" name="randomcode" value="Generate Random Code" />
                        </div>

                        <div class="left">
                            <label>Voucher Price</label>
                            <input type="text" name="V_Price" placeholder="Product Price" />
                        </div>

                        <div class="left">
                            <label>Voucher Available Amount</label>
                            <input type="number" name="V_Amount" placeholder="Available Amount" />
                        </div>
                        ...
                        <table class="table caption-top">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col">Voucher Expried Date</th>
                                </tr>
                            </thead>
                        ...
                        ?>
                                <tr>
                                    ...
                                    <td><?php echo $row["VoucherPrice"] ?></td>
                                    <td><?php echo $row["VoucherStatus"] ?></td>
                                    <td><?php echo $row["VoucherExpiredDate"] ?></td>
                                    <td><a href="A_VoucherEdit.php?id=<?php echo $row["VoucherID"]?>"
